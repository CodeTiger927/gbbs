...
struct dynamic_vertex_data {
  ...
  uintE stored; // how many nodes are actually stored in the sparse table 
...
/**
* The struct that stores all the edges and vertices in the dynamic symmetric 
* graph. It supports dynamic operations(adding and removing edges and vertices)
* and the graph is undirected(symmetric).
*
* To add vertices, use the batchAddVertices function, and to remove vertices, 
* use the batchRemoveVertices function.
* To add edges, use the batchAddEdges function, and to remove edges, use
* the batchRemoveEdges function.
*
* @param W the template parameter that determins the type of the weight on the
* graph
* @param vertex_type the type of vertex data that should be used(it should
* usually just be dynamic_symmetric_vertex)
*/
...
struct dynamic_symmetric_graph {
  ...
  // List of the vertices, indexed by their vertex id.
  ...
  *
  * @param _v_data the list of existing vertices (can be empty if you are 
  * trying to create an empty graph)
  * @param _n the number of vertices in the graph
  * @param _m the number of edges in the graph
  * @param _deletion_fn the function that will be invoked once the struct is to
  * be destructed
  * @param _existVertices this stores whether nodes are actually initialized in
  * the v_data list.
  */
  dynamic_symmetric_graph(pbbslib::dyn_arr<dynamic_vertex_data> & _v_data, 
    size_t _n, size_t _m,
  ...
  */
  ...
  /**
  * Given the expected number of vertices, it would automatically adjust the
  * v_data to an apporpriate capacity
  * 
  * @param amount The new expected number of vertices (or more accurately, the
  * max ID expected to be stored in the graph)
  */
  void adjustVdata(size_t amount) {
    ...
    dynamic_vertex_data* nA = 
    pbbslib::new_array_no_init<dynamic_vertex_data>(nC);
  ...
  /**
  * It would automatically adjust a node's data structures to an appropriate
  * amount (it can both resize up and down)
  * 
  * @param u the index of the node that you want to be adjusted
  */
  void adjustNeighbors(uintE u) {
    size_t amount = std::max((size_t)(v_data.A[u].stored),
      INIT_DYN_GRAPH_EDGE_SIZE);
    size_t cur = v_data.A[u].neighbors.m;
    if((amount << 1) <= cur && (amount << 2) >= cur) return;
 
    auto entries = v_data.A[u].neighbors.entries();

    v_data.A[u].neighbors = 
    make_sparse_table<uintE,bool,hash_uintE>(2 * amount,
      std::make_tuple(UINT_E_MAX,false),hash_uintE());
    par_for(0,entries.size(),1,[&](size_t i) {
      uintE cur = std::get<0>(entries[i]);
      if(std::get<1>(entries[i])) v_data.A[u].neighbors.insert(
        std::make_tuple(cur,true));
    });
  ...
  * 
  * @param vertices the sequence of vertices that you wish to be added
  * into the graph
  */
  void batchAddVertices(sequence<uintE> & vertices) {
    ...
    par_for(0,sumV,1,[&](size_t i) {
      ...
      v_data.A[id].neighbors = 
      make_sparse_table<uintE,bool,hash_uintE>(INIT_DYN_GRAPH_EDGE_SIZE,
        std::make_tuple(UINT_E_MAX,false),hash_uintE());
  ...
  /**
  * Adds a series of edges to the graph in parallel that are centered around a
  * specific node (faster than the other version)
  ...
  */
  void batchAddEdges(uintE u,sequence<uintE> edges) {
    pbbs::sequence<uintE> filteredEdges = pbbs::filter(edges,[&](uintE i){
      return !existEdge(u,i);});
  ...
  * The edges needs to be unique (i.e. (u,v) and (v,u) cannot be added at the
  * same time)
  *
  ...
  */
  void batchAddEdges(sequence<std::pair<uintE,uintE>> edges) {
    ...
    sequence<std::pair<uintE,uintE>> filteredEdges = filter(edges,[&](
      std::pair<uintE,uintE> i){return !existEdge(i.first,i.second);});
  

    this -> m += filteredEdges.size();

    sequence<std::pair<uintE,uintE>> allS = sequence<std::pair<uintE,uintE>>(
      2 * filteredEdges.size(),[&](size_t i) {
      if(i % 2) {
        return std::make_pair(filteredEdges[i / 2].first,
          filteredEdges[i / 2].second);
      }else{
        return std::make_pair(filteredEdges[i / 2].second,
          filteredEdges[i / 2].first);
      }
    });

    // Sorting to recalculate the degree

    // Using merge sort for stability, as it guarentees NlogN work regardless
    // of data specifics.
    sequence<std::pair<uintE,uintE>> all = merge_sort(allS,
    ...
    if(all.size() > 0) v_data.A[all[2 * filteredEdges.size() - 1].first].degree
     += 2 * filteredEdges.size();
    if(all.size() > 0) v_data.A[all[2 * filteredEdges.size() - 1].first].stored
     += 2 * filteredEdges.size();
  ...
  * 
  * @param vertices the sequence of vertices that you wish to be removed from
  * the graph
  */
  ...
  /**
  * Removes a series of edges to the graph in parallel that are centered around
  * a specific node (faster than the other version)
  * The edges needs to be unique
  *
  * @param u the node that these edges are centered around
...
*/
template <template <class W> class vertex_type, class W>
dynamic_symmetric_graph<dynamic_symmetric_vertex,W> 
createEmptyDynamicSymmetricGraph() {

  std::function<void()> doNothing = []() {};
  pbbslib::dyn_arr<dynamic_vertex_data> _vData = 
  pbbslib::dyn_arr<dynamic_vertex_data>(1);
  pbbslib::dyn_arr<bool> tmp = pbbslib::dyn_arr<bool>(1);
  tmp.A[0] = false;
  return dynamic_symmetric_graph<dynamic_symmetric_vertex,W>(_vData,0,0,
    doNothing,tmp);
}
...
* 
* @param W the template parameter that determins the type of the weight
* on the graph
* @param vertex_type the type of vertex data that should be used(it should
* usually just be dynamic_symmetric_vertex)
...
dynamic_symmetric_graph<dynamic_symmetric_vertex,W> dynamifyDSG(Graph G) {
  ...
  dynamic_symmetric_graph<dynamic_symmetric_vertex,W> dsg = 
  createEmptyDynamicSymmetricGraph<dynamic_symmetric_vertex,W>();

  pbbs::sequence<uintE> v = pbbs::sequence<uintE>(G.n,
    [&](size_t i){return i;});
  dsg.batchAddVertices(v);

  for(size_t i = 0; i < G.n;i++) {
    pbbs::sequence<uintE> s = pbbs::sequence<uintE>(G.get_vertex(i).degree,
      [&](size_t j){return G.get_vertex(i).getOutNeighbor(j);});
