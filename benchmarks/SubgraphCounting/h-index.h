...
class HSet2 : public HSet {
  ...
  public:
    ...
    uintE insert(sequence<uintE> batch, bool sorted = false) {
      ...
      //TODO: Adds additional space to C if it is too small
      size_t highestLowC = pbbs::binary_search(deg, [&] (uintE x) {
        return x >= threshold;
      });
      if (deg[highestLowC] <= deg.size()) {
        lowC.size = std::max(lowC.size, (size_t) deg[highestLowC] + 1);
      }


      if (highCStored + sortedBatch.size() >= highC.m) {

        auto entries = pbbs::filter(highC.entries(), [&] (std::tuple<uintE, pbbslib::dyn_arr<uintE>*> element) {
          return std::get<1>(element) != nullptr;
        });
        highC.resize_no_copy(2 * (highCStored + sortedBatch.size()));
        par_for(0, entries.size(), [&] (size_t i) {
          highC.insert(std::make_tuple(std::get<0>(entries[i]), std::get<1>(entries[i])));
        });
    ...
    void addToC(pbbs::sequence<uintE> batch, pbbs::sequence<uintE> deg) {
      ...
      pbbs::sequence<uintE> addedToHighC = pbbs::sequence<uintE>(indices.size());
      ...
      highCStored += pbbslib::reduce_add(addedToHighC);
    }
    ...
    pbbslib::dyn_arr<uintE>* getC(uintE deg) {
      ...
      else if (deg >= lowC.size) return nullptr;
    ...
    pbbs::sequence<uintE> getH() { return pbbs::sequence<uintE>(0); }
    ...
    pbbs::sequence<uintE> getH() {
      ...
      auto H = make_sparse_table<uintE, pbbs::empty,hash_uintE>(2 * this->hindex + 1 ,std::make_tuple(UINT_E_MAX, pbbs::empty()),hash_uintE());
      par_for(this->hindex + 1, lowC.size, [&] (size_t i) {
        if (getC(i) != nullptr) {
          par_for(0, getC(i)->size, [&] (size_t j) {
            H.insert(std::make_tuple(getC(i)->A[j], pbbs::empty()));
          });
        }
      });
